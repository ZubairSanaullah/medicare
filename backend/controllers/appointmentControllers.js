...
export const getAppointments = async (req, res) => {
    try {
        ...
        const items = await Appointment.find(filter).sort({ createdAt: -1 })
            .skip(skip).limit(limit).populate("doctorId", "name specialization owner imageUrl image").lean();
...
export const getAppointmentsByPatient = async (req, res) => {
    try {
        ...
        const resolveCreatedBy = queryCreatedBy || clerkUserId || null;

        console.log('Resolved created by (query or req.auth.userId)', resolveCreatedBy);

        if (!resolveCreatedBy && !req.query.mobile) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized access. Login required to view appointments."
            })
        }
...
export const createAppointment = async (req, res) => {
    try {
        ...
        if (!clerkUserId) return res.status(401).json({
            success: false,
            message: "Unauthorized access. Login required to create appointment."
        });
        ...
        if (existingBooking) {
            return res.status(400).json({
                success: false,
                message: "You already have an appointment with this doctor at this date and time."
            })
        }
...
export const confirmAppointment = async (req, res) => {
    try {
        ...
        console.error("confirmAppointment unexpected:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const updateAppointment = async (req, res) => {
    try {
        ...
        const updated = await Appointment.findByIdAndUpdate(id, update, { new: true, runValidators: true }).populate({
            path: 'doctorId',
...
export const getAppointmentsByDoctor = async (req, res) => {
    try {
        ...
        const items = await Appointment.find(filter).sort({
            date: 1,
            time: 1
        }).skip(skip).limit(limit).populate("doctorId", "name specialization owner imageUrl image").lean();
        ...
        console.error("getAppointmentsByDoctorId unexpected:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}

// To get Register user count 
export async function getRegisterUsersCount(req, res) {
    try {
        ...
        console.error("getRegisterUsersCount unexpected:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
